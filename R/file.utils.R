...
##  Also have intToChar & ASCII from R.oo Version 1.2.7

extract.file.utils.funcs <- function(file=NULL) {
    # have to have R.utils and R.oo attached ahead of trackObjs when running this func
    if (any(!is.element(c("package:R.oo", "package:R.utils"), search())))
        stop("must have packages R.utils and R.oo loaded")
    if (!is.null(file)) {
        sink(file)
        on.exit(sink())
    }
    ...
    cat("\nASCII <-\nc(\n")
    charToInt <- get("charToInt.default", pos="package:R.oo")
    ASCII <- get("ASCII", pos="package:R.oo")
    intToOct <- get("intToOct", pos="package:R.utils")
    cat(paste("\"\\", intToOct(charToInt(ASCII)), "\"", sep=""), fill=65, sep=", ")
    cat(")\n")
    cat("\nintToChar <-\n    ")
    catn(deparse(get("intToChar", pos="package:R.oo"), control=character(0), width.cutoff=75))
}
...
# Keep the ASCII table in the backslash format! (don't change to deparsed version)
ASCII <-
c(
