## Design:
##
## There is a trackingEnv attribute on the tracked
...
##   * trackingDir (absolute path -- don't depend on getwd() -- can be changed -- env attr)
...
track.start <- function(dir, pos=1, envir=as.environment(pos),
                         create=TRUE, clobber=c("no", "files", "variables"),
                         cache=NULL, options=NULL, RDataSuffix=NULL) {
    ...
    if (env.is.tracked(envir))
        stop("env ", envname(envir), " is already tracked by dir '",
             attr(getTrackingEnv(envir), "trackingDir"), "'")
    ...
    attr(trackingEnv, "trackingDir") <- dir
...
env.is.tracked <- function(pos=1, envir=as.environment(pos)) {
    return(is.element("trackingEnv", names(attributes(envir))))
}
...
track <- function(expr, pos=1, envir=as.environment(pos), list=NULL, pattern=NULL, glob=NULL, all=FALSE) {
    ...
    if (length(list)) {
        ...
        if (any(alreadyTracked)) {
            cat("the following objects are already tracked: ",
...
isReservedName <- function(objname)
    ...
    return((regexpr("[\n\r]", objname) > 0)
           | is.element(objname, c(".trackingEnv", ".trackingFileMap", ".trackingUnsaved", ".trackingSummary", ".trackingSummaryChanged", ".trackingOptions")))
...
getTrackingDir <- function(trackingEnv) {
    ...
    if (is.element("trackingDir", names(attributes(trackingEnv))))
        dir <- attr(trackingEnv, "trackingDir")
    if (is.null(dir))
        stop("trackingEnv ", envname(trackingEnv), " has no 'trackingDir' attribute")
    if (!is.character(dir) || length(dir)!=1)
        stop("attribute 'trackingDir' on trackingEnv ", envname(trackingEnv),
...
setTrackingEnv <- function(trackedEnv, trackingEnv) {
    ...
    attr(trackedEnv, "trackingEnv") <- trackingEnv
    invisible(NULL)
}
...
getTrackingEnv <- function(trackedEnv) {
    env <- NULL
    ## avoid partial name matching for attributes...
    if (is.element("trackingEnv", names(attributes(trackedEnv))))
        env <- attr(trackedEnv, "trackingEnv")
    if (is.null(env))
        stop("env ", envname(trackedEnv), " is not tracked (has no 'trackingEnv' attribute)")
    if (!is.environment(env))
        stop("attribute 'trackingEnv' on env ", envname(trackedEnv),
